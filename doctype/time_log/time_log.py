...
class TimeLog(Document):
	def validate(self):
		...
		self.set_contract_if_missing()
		self.update_cost()

	def on_submit(self):
		self.update_production_order()
		self.update_task_and_contract()
	...
	def set_title(self):
		...
		elif self.activity_type and (self.task or self.contract):
			self.title = _("{0} for {1}").format(self.activity_type, self.task or self.contract)
		else:
			self.title = self.task or self.contract or get_fullname(frappe.session.user)
	...
	def set_contract_if_missing(self):
		"""Set contract if task is set"""
		if self.task and not self.contract:
			self.contract = frappe.db.get_value("Task", self.task, "contract")
	...
	def update_task_and_contract(self):
		"""Update costing rate in Task or Contract if either is set"""
		...
		if self.task:
			...
			frappe.get_doc("Contract", self.contract).update_contract()
...
def get_events(start, end, filters=None):
	...
	:param filters: Filters like workstation, contract etc.
	"""
	from frappe.desk.calendar import get_event_conditions
	conditions = get_event_conditions("Time Log", filters)

	data = frappe.db.sql("""select name, from_time, to_time,
		activity_type, task, contract, production_order, workstation from `tabTime Log`
	...
	for d in data:
		...
		if d.contract:
			d.title += " for Contract: " + d.contract
