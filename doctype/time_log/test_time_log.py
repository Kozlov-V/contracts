...
from erpnext.contracts.doctype.time_log.time_log import OverlapError, NotSubmittedError, NegativeHoursError
...
class TestTimeLog(unittest.TestCase):
	...
	def test_default_activity_cost(self):
		...
		contract_name = "_Test Contract for Activity Type"

		frappe.db.sql("delete from `tabTime Log` where contract=%s or employee='_T-Employee-0002'", contract_name)
		frappe.delete_doc("Contract", contract_name)
		contract = frappe.get_doc({"doctype": "Contract", "contract_name": contract_name}).insert()

		make_time_log_test_record(employee="_T-Employee-0002", hours=2,
			activity_type = "_Test Activity Type", contract = contract.name)

		contract = frappe.get_doc("Contract", contract.name)
		self.assertTrue(contract.total_costing_amount, 30)
		self.assertTrue(contract.total_billing_amount, 40)

	def test_total_activity_cost_for_contract(self):
		frappe.db.sql("""delete from `tabTask` where contract = "_Test Contract 1" """)
		frappe.db.sql("""delete from `tabContract` where name = "_Test Contract 1" """)
		frappe.db.sql("""delete from `tabTime Log` where name = "_Test Contract 1" """)
		...
		frappe.get_doc({
			"contract_name": "_Test Contract 1",
			"doctype": "Contract",
			"tasks" :
				[{ "title": "_Test Contract Task 1", "status": "Open" }]
		}).save()

		task_name = frappe.db.get_value("Task",{"contract": "_Test Contract 1"})
		...
		self.assertEqual(frappe.db.get_value("Contract", "_Test Contract 1", "total_billing_amount"), 200)
		...
		self.assertEqual(frappe.db.get_value("Contract", "_Test Contract 1", "total_billing_amount"), 400)
...
def make_time_log_test_record(**args):
	...
	time_log.contract = args.contract
