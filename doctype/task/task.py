...
class Task(Document):
	...
	def update_total_expense_claim(self):
		self.total_expense_claim = frappe.db.sql("""select sum(total_sanctioned_amount) from `tabExpense Claim`
			where contract = %s and task = %s and approval_status = "Approved" and docstatus=1""",(self.contract, self.name))[0][0]
	...
	def update_contract(self):
		if self.contract and not self.flags.from_contract:
...
def get_events(start, end, filters=None):
	...
	data = frappe.db.sql("""select name, exp_start_date, exp_end_date,
		subject, status, contract from `tabTask`
...
def get_contract(doctype, txt, searchfield, start, page_len, filters):
	from erpnext.controllers.queries import get_match_cond
	return frappe.db.sql(""" select name from `tabContract`
